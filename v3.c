...
#define PS_ADDR 0x33
#define PM_THRESHOLD 2
#define RH_THRESHOLD 50
...
#define BOOT_DELAY 5000000  /* in microsecons */
#define SCREEN_DELAY 5000  /* in milliseconds */
#define POLL_INTERVAL 3000000 /* in microseconds */

#define ALARM_DURATION 5000000 /* in microseconds */
#define SUPP_TIME 60000000
#define ADVANCED_MODE_DETECTED 5000000
...
uint8_t display_mode, suppress, interrupted;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  if(GPIO_Pin == YELL_BUTTON_PIN)
  {
    ...
    if(display_mode == DATA_MODE) /* toggle to ALERT_MODE */
    {
                  ...
                  struct ps_data_t *ps, uint8_t mode)
{
  ...
  {
    sprintf(items[0], "%s %s", LABEL2, rh);
    sprintf(items[1], "%s %s", LABEL1, temp);
    ...
    for(i = 0; i < NUM_LINES; i++)
    {
      ...
      init_lcd();
      send_lcd_command(LCD_ROW1);
      print_message(items[j]);
      send_lcd_command(LCD_ROW2);

      print_message(items[(j + 1) % NUM_LINES]);
